...
cat /etc/wildydev21/xray-mini-utils/socks-backup.json | jq 'del(.inbounds[0].settings.accounts[] | select(.user == "'${client}'"))' > /etc/xray-mini/socks.json;
rm -rf /etc/wildydev21/socks/${client};

...
cat /etc/wildydev21/xray-mini-utils/tls-backup.json | jq 'del(.inbounds[0].settings.clients[] | select(.password == "'${client}'"))' > /etc/wildydev21/xray-mini-cache.json;
cat /etc/wildydev21/xray-mini-cache.json | jq 'del(.inbounds[1].settings.clients[] | select(.password == "'${client}'"))' > /etc/wildydev21/xray-mini-cache2.json;
cat /etc/wildydev21/xray-mini-cache2.json | jq 'del(.inbounds[4].settings.clients[] | select(.password == "'${client}'"))' > /etc/xray-mini/tls.json;
rm -rf /etc/wildydev21/trojan/${client};

...
cat /etc/wildydev21/xray-mini-utils/tls-backup.json | jq 'del(.inbounds[2].settings.clients[] | select(.email == "'${client}'"))' > /etc/wildydev21/xray-mini-cache.json;
cat /etc/wildydev21/xray-mini-cache.json | jq 'del(.inbounds[5].settings.clients[] | select(.email == "'${client}'"))' > /etc/xray-mini/tls.json;
cp /etc/xray-mini/nontls.json /etc/wildydev21/xray-mini-utils/nontls-backup.json;
cat /etc/wildydev21/xray-mini-utils/nontls-backup.json | jq 'del(.inbounds[0].settings.clients[] | select(.email == "'${client}'"))' > /etc/xray-mini/nontls.json;
rm -rf /etc/wildydev21/vmess/${client};

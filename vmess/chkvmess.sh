...
echo -n > /etc/wildydev21/cache/vmess_temp.txt;
echo -n > /etc/wildydev21/cache/vmess_temp2.txt;
...
if [[ "$jum" = "$ip" ]]; then
echo "$jum" >> /etc/wildydev21/cache/vmess_temp.txt;
else
echo "$ip" >> /etc/wildydev21/cache/vmess_temp2.txt;
fi
jum2=$(cat /etc/wildydev21/cache/vmess_temp.txt);
sed -i "/$jum2/d" /etc/wildydev21/cache/vmess_temp2.txt > /dev/null 2>&1;
done
jum=$(cat /etc/wildydev21/cache/vmess_temp.txt);
if [[ -z "$jum" ]]; then
echo > /dev/null;
else
jum2=$(cat /etc/wildydev21/cache/vmess_temp.txt | nl);
...
echo "=============================================";
echo -n > /etc/wildydev21/cache/vmess_temp3.txt;
data=(`cat /etc/xray-mini/client.conf | grep '^Vmess' | cut -d " " -f 2`);
for akun in "${data[@]}"
do
if [[ -z "$akun" ]]; then
...
ip=$( cat /etc/wildydev21/xray-mini-nontls/access.log | grep "$(date -d "0 days" +"%H:%M" )" | grep -w $akun | tail -n100 | awk '{print $3}' | cut -d: -f1 | sort | uniq );
if [[ -z "$ip" ]]; then
echo > /dev/null
else
jum5=$(echo $ip | nl);
...
echo "=============================================";
oth=$(cat /etc/wildydev21/cache/vmess_temp2.txt | sort | uniq | nl )
echo "GRPC IP : All User IP";
echo "$oth";
echo "=============================================";
echo "";
rm -rf /etc/wildydev21/cache/vmess_temp2.txt;
rm -rf /etc/wildydev21/cache/vmess_temp.txt;
rm -rf /etc/wildydev21/cache/vmess_temp3.txt;
...
cat /etc/wildydev21/xray-mini-utils/shadowsocks-backup.json | jq 'del(.inbounds[0].settings.clients[] | select(.email == "'${client}'"))' > /etc/xray-mini/shadowsocks.json;
rm -rf /etc/wildydev21/shadowsocks/${client};

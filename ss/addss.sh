...
cp /etc/xray-mini/shadowsocks.json /etc/wildydev21/xray-mini-utils/shadowsocks-backup.json;
cat /etc/wildydev21/xray-mini-utils/shadowsocks-backup.json | jq '.inbounds[0].settings.clients += [{"method": "chacha20-ietf-poly1305","password": "'${Username}'","email": "'${Username}'" }]' > /etc/xray-mini/shadowsocks.json;
...
mkdir -p /etc/wildydev21/shadowsocks/;
mkdir -p /etc/wildydev21/shadowsocks/${Username};
rm -f /etc/wildydev21/shadowsocks/${Username}/config.log;

# // Success
sleep 1;
clear;
echo -e "Your Premium Shadowsocks Details" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e "===============================" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e " Remarks     = ${Username}" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e " IP          = ${IP_NYA}" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e " Address     = ${domain}" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e " Port        = ${port}" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e " Password    = ${Username}" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e "===============================" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e " SHADOWSOCKS CONFIG LINK" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e ' ```'${link_config}'```' | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e "===============================" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e " Created     = ${hariini}" | tee -a /etc/wildydev21/shadowsocks/${Username}/config.log;
echo -e " Expired     = ${exp}";
echo -e "===============================";
...
grep -c -E "^#DEPAN " "/etc/wireguard/wg0.conf" > /etc/wildydev21/jumlah-akun-wireguard.db;
grep "^#DEPAN " "/etc/wireguard/wg0.conf" | cut -d ' ' -f 4  > /etc/wildydev21/akun-wireguard.db;
totalaccounts=`cat /etc/wildydev21/akun-wireguard.db | wc -l`;
echo "Total Akun = $totalaccounts" > /etc/wildydev21/total-akun-wireguard.db;
for((i=1; i<=$totalaccounts; i++ ));
do
    # // Username Interval Counting
    username=`head -n $i /etc/wildydev21/akun-wireguard.db | tail -n 1`;
...
if [[ $sisa_hari -lt 1 ]]; then
    ...
    rm -f /etc/wildydev21/webserver/wg-client/$client.conf;
    rm -rf /etc/wildydev21/wireguard/$client;
    ...
    echo "Username : $username | Expired : $expired | Deleted $now" >> /etc/wildydev21/wireguard-expired-deleted.db;

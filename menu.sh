...
echo -e "Current Domain      = $( cat /etc/wildydev21/domain.txt )";
...
case $choosemu in
    ...
    16) 
        ...
        # // Input Domain To VPS
        echo "$new_domains" > /etc/wildydev21/domain.txt;
    ...
    18)
        ...
        echo $email_input > /etc/wildydev21/email.txt
        clear;
    ...
    ;;
    28) cd /root/; wget -q -O /root/update.sh "https://releases.wildydev21.com/vpn-script/Stable/update.sh"; chmod +x /root/update.sh; ./update.sh; rm -f /root/update.sh ;;
    29)
            clear
            # // clearlog

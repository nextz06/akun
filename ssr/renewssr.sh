...
# // Start
CLIENT_001=$(grep -c -E "^SSR " "/etc/wildydev21/ssr-client.conf" );
echo "    ==================================================";
echo "           LIST SHADOWSOCKSR CLIENT ON THIS VPS";
echo "    ==================================================";
grep -e "^SSR " "/etc/wildydev21/ssr-client.conf" | cut -d ' ' -f 2-3 | nl -s ') '
...
client=$(grep "^SSR " "/etc/wildydev21/ssr-client.conf" | cut -d ' ' -f 2 | sed -n "${CLIENT_002}"p);
expired=$(grep "^SSR " "/etc/wildydev21/ssr-client.conf" | cut -d ' ' -f 3 | sed -n "${CLIENT_002}"p);
port=$(grep "^SSR " "/etc/wildydev21/ssr-client.conf" | cut -d ' ' -f 4 | sed -n "${CLIENT_002}"p);
...
sed -i "/\b$client\b/d" /etc/wildydev21/ssr-client.conf;
echo -e "SSR $client $exp4 $port" >> /etc/wildydev21/ssr-client.conf;

...
cat /etc/wildydev21/xray-mini-utils/http-backup.json | jq 'del(.inbounds[0].settings.accounts[] | select(.user == "'${client}'"))' > /etc/xray-mini/http.json;
rm -rf /etc/wildydev21/http/${client};
